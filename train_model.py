...
from ml.model import train_model, save_model_artifacts, \
...
def go():
    ...
    # Preprocess the data using one hot encoding for the categorical features
    # and a label binarizer for the labels
    X_train, y_train, encoder, lb = process_data(
        train, categorical_features=cat_features, label="salary", training=True
    )
    logger.info("Data preprocessing of training data complete")
    ...
    save_model_artifacts(
        os.path.join(os.getcwd(), "model_artifacts"), rf_model, encoder, lb
    )
    logger.info("Model artifacts saved")

    # Load the model artifacts
    # tmp_rf_model, tmp_encoder, tmp_lb = load_model_artifacts(
    ...
    logger.info(f"Training data performance (precision recall fbeta): "
                f"{precision:.3f}\t{recall:.3f}\t{fbeta:.3f}")
    ...
    logger.info(f"Test data performance (precision recall fbeta): "
                f"{precision:.3f}\t{recall:.3f}\t{fbeta:.3f}")
    ...
    logger.info("slice_output.txt saved")
